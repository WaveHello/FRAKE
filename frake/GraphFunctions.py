...
def PlotAll(DATA, IDs, xx, yy, style='Paper', xlabel='default', ylabel='default', xsize=3, ysize=3, mode='-', hold=0 \
            , palete='Normal', legendMode=True, lims=[0,0,0,0,0], varst=0, PlotName='default', sample_span=1\
               , linewith=0, markers=0, makersize=0, data_selection = [0, -1]):
    ...
    for Frame in DATA:
        ...
        if (style=='Paths'):
            ...
            if (PlotName=='default'):
                ...
                plt.plot(Frame[xx][data_selection[0]:data_selection[1]:sample_span],Frame[yy][data_selection[0]:data_selection[1]:sample_span], aux_mode, label=IDs[a-1],\
                         linewidth=lw,color=Pt[a-1])
            else:
                PlotName.plot(Frame[xx][data_selection[0]:data_selection[1]:sample_span],Frame[yy][data_selection[0]:data_selection[1]:sample_span], aux_mode, label=IDs[a-1],\
                              linewidth=lw,color=Pt[a-1])
